...
def fit_predict_nn(Xtr, Rtr, ltr, Xte, lte, seed):
    ...
    for _ in range(NN_EPOCHS):
        ...
        for i in range(0, len(Str), NN_BATCH):
            ...
            loss = F.mse_loss(torch.sigmoid(s), y[b]) + 0.1 * -(target[b] * F.log_softmax(s, 1)).sum(1).mean()
            opt.zero_grad()
            loss.backward()
            opt.step()
            sched.step()
    model.eval()
    with torch.no_grad():
        return torch.sigmoid(model(seq_te, st_te, ld_te)).cpu().numpy()
...
def fit_predict_lgb(Xtr, Rtr, ltr, Xte, lte):
    ...
    for rs, s in [(r, s) for r in range(3) for s in range(5)]:
        m = lgb.LGBMRegressor(n_estimators=LGB_ROUNDS, learning_rate=0.03, num_leaves=15, min_child_samples=20,
                              subsample=0.8, subsample_freq=1, colsample_bytree=0.5, random_state=s + 7 * rs,
