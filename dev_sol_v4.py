...
LGB_ROUNDS = 300
RANK_ROUNDS = 400
RANK_GAINS_ASC = [0.10, 0.25, 0.45, 0.70, 1.00]
KNN_PCA = 40
KNN_K = 40
...
def long_features(X, l):
    n = len(X)
    _, St = site_stats(X)
    rel = St / (np.abs(St).mean(1, keepdims=True) + 1e-6)
    mean_stats = St.mean(1)
    eye = np.eye(5, dtype=np.float32)
    return np.concatenate([St.reshape(n * 5, -1), rel.reshape(n * 5, -1), np.repeat(mean_stats, 5, axis=0),
                           np.tile(eye, (n, 1)), np.repeat(l[:, None], 5, axis=0)], 1).astype(np.float32)


def fit_predict_rank(Xtr, Rtr, ltr, Xte, lte):
    A, B = long_features(Xtr, ltr), long_features(Xte, lte)
    label = (4 - np.argsort(np.argsort(-Rtr, 1), 1)).reshape(-1).astype(int)
    m = lgb.LGBMRanker(objective="lambdarank", label_gain=RANK_GAINS_ASC, eval_at=[5], n_estimators=RANK_ROUNDS,
                       learning_rate=0.03, num_leaves=15, min_child_samples=20, subsample=0.8, subsample_freq=1,
                       colsample_bytree=0.5, n_jobs=4, deterministic=True, force_row_wise=True, verbose=-1,
                       lambdarank_truncation_level=5, random_state=0)
    m.fit(A, label, group=np.full(len(Xtr), 5))
    return m.predict(B).reshape(-1, 5).astype(np.float32)
...
def fit_predict(Xtr, Rtr, ltr, Xte, lte):
    members = {
        ...
        "rank": fit_predict_rank(Xtr, Rtr, ltr, Xte, lte),
    }
