import os; os.environ.setdefault("OMP_NUM_THREADS", "1"); os.environ.setdefault("OMP_WAIT_POLICY", "PASSIVE")
"""KEY=err (namespace err_k)  targeted feature/model ideas from the error analysis. Usage: dev_wf_err_b.py idea1,idea2,..."""
...
def lgb_reg(A, y, B, seed=0, n_est=300):
    m = lgb.LGBMRegressor(n_estimators=n_est, learning_rate=0.03, num_leaves=15, min_child_samples=20, subsample=0.8,
                          subsample_freq=1, colsample_bytree=0.5, random_state=seed, n_jobs=1, deterministic=True,
    ...
    for tr, va in FOLDS:
        for s in range(5):
            m = lgb.LGBMClassifier(n_estimators=200, learning_rate=0.03, num_leaves=15, min_child_samples=20,
                                   subsample=0.8, subsample_freq=1, colsample_bytree=0.5, random_state=s, n_jobs=1,
...
def run_long(Lf, name):
    ...
    for tr, va in FOLDS:
        ...
        m = lgb.LGBMRegressor(n_estimators=600, learning_rate=0.03, num_leaves=31, min_child_samples=20, subsample=0.8,
                              subsample_freq=1, colsample_bytree=0.5, random_state=0, n_jobs=1, deterministic=True,
                              force_row_wise=True, verbose=-1)
...
if __name__ == "__main__":
    ...
    for idea in ideas:
        if idea == "type":  # baseline + type-separated evidence-proxy features + in-row ranks
            run_wide(np.concatenate([base, per, rk, rel], 1), "err__k_type")
        elif idea == "typeonly":
            run_wide(np.concatenate([per, rk, rel, lead[:, None]], 1), "err__k_typeonly")
        elif idea == "cls":
            run_cls(np.concatenate([base, per, rk, rel], 1), "err__k_type_cls")
        elif idea == "long":
            K = per.shape[1] // 5
