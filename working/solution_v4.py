...
KNN_PCA = 40
KNN_K = 40
BLEND = {"nn": 0.3, "lgb": 0.2, "knn": 0.1, "et": 0.4}
...
def fit_predict_et(Xtr, Rtr, ltr, Xte, lte):
    A = np.concatenate([flat_features(Xtr, ltr), cross_site_features(Xtr)], 1)
    B = np.concatenate([flat_features(Xte, lte), cross_site_features(Xte)], 1)
    m = ExtraTreesRegressor(n_estimators=ET_TREES, min_samples_leaf=ET_LEAF, max_features=ET_FEATS,
                            n_jobs=4, random_state=0)
    m.fit(A, Rtr)
    return m.predict(B).astype(np.float32)
...
def fit_predict(Xtr, Rtr, ltr, Xte, lte):
    members = {
        ...
        "et": fit_predict_et(Xtr, Rtr, ltr, Xte, lte),
    }
